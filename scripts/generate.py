...
def build_prompt(user_prompt: str, size_px: str | None, quality: str, has_ref: bool) -> str:
    ...
    # Neutral: just ensure the attached image is used. The prompt itself decides
    # HOW (style transfer for a new subject, or edit while keeping the subject).
    ref_line = (
        "Use the attached image(s) as the reference for the request below; the "
        "request decides how to use them — e.g. match their style for a new "
        "subject, or edit them while keeping the subject.\n\n"
        if has_ref else ""
    )
    return (
